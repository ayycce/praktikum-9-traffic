...
# 2. INIT SPARK
# Cache session agar tidak perlu reload mesin Spark tiap kali klik tombol
@st.cache_resource
def get_spark():
...
# Mengambil 3 jenis data hasil olahan engine
try:
...
# 6. VISUALIZATION & AI PREDICTION
c1, c2 = st.columns(2)

with c1:
    st.subheader("📈 Traffic Time Series")
    # Memperbaiki format waktu agar bisa dibaca grafik Plotly
...
with c2:
    ...
    # Melatih AI secara instan dengan Regresi Linear
    model = LinearRegression()
    model.fit(X, y)
    
    # Membuat slider agar user bisa pilih jam (0 - 23)
    hour_input = st.slider("Pilih Jam untuk Prediksi:", 0, 23, 12)
    pred = model.predict([[hour_input]])
    
    st.success(f"Prediksi jumlah kendaraan pada jam **{hour_input}:00** adalah **{int(pred[0])}** kendaraan 🚗")